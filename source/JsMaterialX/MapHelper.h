...
struct BindingType<std::map<std::string, T>> {
    ...
    static WireType toWireType(const std::map<std::string, T> &map, rvp::default_tag = {}) {        
        val obj = val::object();
...
struct BindingType<std::unordered_map<std::string, T>> {
    ...
    static WireType toWireType(const std::unordered_map<std::string, T> &map, rvp::default_tag = {}) {        
        val obj = val::object();
        for (std::pair<std::string, T> element : map)
        {
            obj.set(element.first, element.second);
